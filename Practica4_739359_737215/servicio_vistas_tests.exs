...
# FECHA: 1/12/2018
# TIEMPO: 15 horas
# DESCRIPCIÓN: Fichero encargado de conectarse con los otros nodos, mandarles
#              el código, y realizar los tests para comprobar el correcto funcionamiento
#              del programa.
...
defmodule  GestorVistasTest do
    ...
    setup_all do
        ...
        #maquinas = ["127.0.0.1", "155.210.154.198", 
        #            "155.210.154.198", "155.210.154.198"] 
        maquinas = ["127.0.0.1"]
    ...
    #@tag :deshabilitado
    test "Primario rearrancado tratado como caido y es convertido en nodo en espera", %{c1: c1, c2: c2, c3: c3} do
        IO.puts("Test: Primario rearrancado tratado como caido y es convertido en nodo en espera ...")

        {vista, _} = ClienteGV.latido(c2, 0)   # nuevo servidor en espera
        IO.inspect rearrancado_caido(c1, c3, 5, ServidorGV.latidos_fallidos() * 2)

        # vista valida debería ser 5
        {vista, _} = ClienteGV.latido(c2, vista.num_vista)

        comprobar_valida(c1, c1, c3, vista.num_vista)
        comprobar_lista_espera(vista, c2, true)
    ...
    test "Servidor de vistas espera a que primario confirme vista pero este no lo hace.", %{c1: c1, c2: c2, c3: c3} do
        IO.puts("Servidor de vistas espera a que primario confirme vista pero este no lo hace ...")
    ...
    #@tag :deshabilitado
    test "Si anteriores servidores caen (Primario  y Copia),
      un nuevo servidor sin inicializar no puede convertirse en primario.", %{c1: c1, c2: c2, c3: c3} do
        IO.puts("Si anteriores servidores caen (Primario  y Copia) ...")
        
        ClienteGV.latido(c1, 0)
        #{vista, _} = ClienteGV.latido(c2, 0)
        sin_inicializar_no(c1, c2, c3, ServidorGV.latidos_fallidos() * 2)
    ...
    defp rearrancado_caido(_c1, _c3, _num_vista_valida, 0), do: :fin
    defp rearrancado_caido(c1, c3, num_vista_valida, x) do
        ClienteGV.latido(c3, num_vista_valida)
        {vista, _} = ClienteGV.latido(c1, num_vista_valida)

        if (vista.primario != c1) or (vista.copia != c3) do
            Process.sleep(ServidorGV.intervalo_latidos())
            rearrancado_caido(c1, c3, num_vista_valida, x-1)
        end
    end

    defp primario_no_confirma_vista(_c1, _c2, _c3, _num_vista_valida, 0), do: :fin
    defp primario_no_confirma_vista(c1, c2, c3, num_vista_valida, x) do
        {vista, _} = ClienteGV.latido(c1, num_vista_valida)
        ClienteGV.latido(c1, num_vista_valida)  #confirman todos menos el primario

        if (vista.primario != :error) and (vista.copia != :error) do
            Process.sleep(ServidorGV.intervalo_latidos())
            primario_no_confirma_vista(c1, c2, c3, num_vista_valida, x-1)
        end
    end

    defp sin_inicializar_no(_c1, _c2, _c3, 0), do: :fin
    defp sin_inicializar_no(c1, c2, c3, x) do
        {vista, _} = ClienteGV.latido(c2, 0)
        Process.sleep(ServidorGV.intervalo_latidos())

        if (vista.primario != :error) or (vista.copia != :error) do
            Process.sleep(ServidorGV.intervalo_latidos())
            sin_inicializar_no(c1, c2, c3, x-1)
        end
        if (vista.primario == :error) or (vista.copia == :error) do
            IO.puts("Vista al caer: #{vista.primario}, #{vista.copia}")
        end    
    end

    defp comprobar_lista_espera(vista, c2, false), do: :fin
    defp comprobar_lista_espera(vista, c2, esta) do
      if(!Enum.member?(vista, c2)) do
          comprobar_lista_espera(vista, c2, false)
      end
    end

    defp comprobar_lista_espera(vista, c2) do
      if(!Enum.member?(vista, c2)) do
          comprobar_lista_espera(vista, c2)
      end
    end
